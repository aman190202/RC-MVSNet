...
class Conv3d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 relu=True, bn=True, bn_momentum=0.1, num_groups=8,init_method="xavier",norm="batch_norm", **kwargs):
        ...
        self.bn = nn.BatchNorm3d(out_channels, momentum=bn_momentum) if norm == "batch_norm" else None
        self.relu = relu
...
def homo_warping(src_fea, src_proj, ref_proj, depth_values):
    ...
    warped_src_fea = F.grid_sample(src_fea, grid.view(batch, num_depth * height, width, 2), mode='bilinear',
                                   padding_mode='zeros',align_corners=True)# align_corners = False
